...
For coprime integers $1<n<d$ with $n$ odd, consider the periodic two-frequency
profile
  $F_k (x) = k cos(2 pi d x) + (1-k) cos(2 pi n x)$, where $k in [0,1]$ is real and $x in RR \/ ZZ$ is phase on the unit circle.
...
The rounded configuration is balanced at every cyclic span length: every
