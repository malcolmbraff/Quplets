...
#nonum[$ Σ_n = { q in ℝ^n_+ : sum q_i = 1 } $].
•	Regular tuplet. #nonum[$ t_n = ( 1/n, …, 1/n ) in Σ_n $].
...
$ Q(n,d) = t_n + λ(d) · m_(n,r) "   with   "  λ(d) = r #h(4pt) ( n - r ) / ( n d ) $<colinearity>

Proof (one line). 

In $E(n,d)$ there are $r$ long gaps of size $(s+1)/d$ and $n-r$ short gaps of size $s/d$.

For $j in L$: 
$ Q_j - 1/n = (s+1)/d - 1/n = (n-r)/(n d) = λ(d) · (1/r) . $
For $j in S$: 
$ Q_j - 1/n = s/d - 1/n = - r/(n d) = λ(d) · ( - 1/(n-r) ) $ ∎

Consequences.

- Colinearity. For fixed $n$ and residue $r$, all $Q(n, s n + r)$ lie on the same line $t_n + λ m_(n,r) $ in $Σ_n$ (same direction $m$, only $λ$ varies).
- Along $d$ in steps of $n$. For $i in {0,1,2,…}$, #nonum[$ Q(n, d + i n) = t_n + [ r ( n - r ) / ( n ( d + i n ) ) ] · m_(n,r) $]
  the offset from $t$ decays like $1/(d + i n)$.
...
#nonum[$ 0 ≤ λ ≤ λ_(n,r) := min{ (n - r)/n, ; r ( 1 - 1/n ) } ,$]
define the λ–quplet
$ Q_lambda (n,r) := t_n + λ · m_(n,r) . $<qlambda>

- For the discrete family $d = s n + r$, we have $λ(d) = r ( n - r ) / ( n d ) in (0, λ_(n,r))$, hence $Q_(lambda(d))(n,r) = Q(n,d)$.
- Allowing arbitrary real $λ$ in $[0, λ_*(n,r)]$ yields a continuous one-parameter curve in $Σ_n$: a smooth morph from the uniform tuplet $t_n$ ($λ=0$) towards increasingly contrasted patterns ($λ$ increasing). For irrational $λ$, the entries of $Q_lambda (n,r)$ are typically irrational — these are “irrational quplets” extending the Euclidean family.
...
$λ(d) = 12 / ( 7 d )$.
- For $d = 10$: #h(5pt) $λ = 12 / 70$;#h(5pt) levels $(s+1)/d = 2/10$ on $L$ and $s/d = 1/10$ on $S$.
- For $d = 17$:#h(5pt) $λ = 12 / ( 7 · 17 )$;#h(5pt) levels $2/17$ and $1/17$;#h(5pt) same order (same $L$), same line.
